...
final class LoggerTests: XCTestCase {
    ...
    func testParseTestNameWithInvalidTestName() throws {
        ...
        // When calling parseTestName
        do {
            let _ = try Logger.parseTestName(testName)
    ...
    func testURLResponseStatusLogging() {
        ...
        let code = 401
        let url = URL(string: "https://example.com")!
        let response = HTTPURLResponse.init(url: url, statusCode: code, httpVersion: nil, headerFields: nil)
        let request = URLRequest(url: url)
    ...
    func testURLResponseHeaderLogging() {
        ...
        let value = UUID().uuidString
        let url = URL(string: "https://example.com")!
        let response = HTTPURLResponse.init(url: url, statusCode: 200, httpVersion: nil, headerFields: ["key": value])
        let request = URLRequest(url: url)
